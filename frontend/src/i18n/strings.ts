export const uiStrings: Record<string, Record<string, string>> = {
  'en': {
    results: 'Past Results', planned: 'Planned Tests', panels: 'Panels',
    biomarker: 'Biomarker', value: 'Value', reference: 'Reference',
    back: '← Back', noResults: 'No blood tests recorded yet.',
    noPlanned: 'No planned tests.', noPanels: 'No panels defined.',
    overdue: 'Overdue', biomarkers: 'biomarkers', analyses: 'analyses',
    navResults: 'Results', navPanels: 'Panels', navAnalytics: 'Analytics',
    navPlanning: 'Planning', navUpcoming: 'Upcoming',
    readMore: 'Read more', showLess: 'Show less', frequencyPrefix: 'Do it',
    signIn: 'Sign in', guest: 'Guest',
    planningTitle: 'Testing Schedule', upcomingTitle: 'Upcoming Tests',
    all: 'All', collapseExpandAll: 'Collapse / Expand all',
    notTracked: 'Not tracked', monthly: 'Monthly', quarterly: 'Quarterly',
    twiceAYear: 'Twice a year', annually: 'Annually', every2Years: 'Every 2 years',
    inherited: 'from panel', clearOverride: 'Clear', trackingCount: 'tracking',
    signInToSchedule: 'Sign in to set up your testing schedule.',
  },
  'ru-RU': {
    results: 'Результаты', planned: 'Запланировано', panels: 'Панели',
    biomarker: 'Показатель', value: 'Значение', reference: 'Норма',
    back: '← Назад', noResults: 'Нет записей анализов.',
    noPlanned: 'Нет запланированных анализов.', noPanels: 'Панели не определены.',
    overdue: 'Просрочено', biomarkers: 'показателей', analyses: 'анализов',
    navResults: 'Результаты', navPanels: 'Панели', navAnalytics: 'Аналитика',
    navPlanning: 'Планирование', navUpcoming: 'Предстоящие',
    readMore: 'Показать больше', showLess: 'Свернуть', frequencyPrefix: 'Сдавать',
    signIn: 'Войти', guest: 'Гость',
    planningTitle: 'График анализов', upcomingTitle: 'Предстоящие анализы',
    all: 'Все', collapseExpandAll: 'Свернуть / Развернуть все',
    notTracked: 'Не отслеживается', monthly: 'Ежемесячно', quarterly: 'Раз в квартал',
    twiceAYear: 'Раз в полгода', annually: 'Раз в год', every2Years: 'Раз в 2 года',
    inherited: 'от панели', clearOverride: 'Сбросить', trackingCount: 'отслеживается',
    signInToSchedule: 'Войдите, чтобы настроить график анализов.',
  },
  'uk-UA': {
    results: 'Результати', planned: 'Заплановано', panels: 'Панелі',
    biomarker: 'Показник', value: 'Значення', reference: 'Норма',
    back: '← Назад', noResults: 'Немає записів аналізів.',
    noPlanned: 'Немає запланованих аналізів.', noPanels: 'Панелі не визначені.',
    overdue: 'Прострочено', biomarkers: 'показників', analyses: 'аналізів',
    navResults: 'Результати', navPanels: 'Панелі', navAnalytics: 'Аналітика',
    navPlanning: 'Планування', navUpcoming: 'Майбутні',
    readMore: 'Показати більше', showLess: 'Згорнути', frequencyPrefix: 'Здавати',
    signIn: 'Увійти', guest: 'Гість',
    planningTitle: 'Графік аналізів', upcomingTitle: 'Майбутні аналізи',
    all: 'Усі', collapseExpandAll: 'Згорнути / Розгорнути все',
    notTracked: 'Не відстежується', monthly: 'Щомісяця', quarterly: 'Раз на квартал',
    twiceAYear: 'Раз на півроку', annually: 'Раз на рік', every2Years: 'Раз на 2 роки',
    inherited: 'від панелі', clearOverride: 'Скинути', trackingCount: 'відстежується',
    signInToSchedule: 'Увійдіть, щоб налаштувати графік аналізів.',
  },
};
